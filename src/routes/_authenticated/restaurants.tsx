...
function RestaurantsPage() {
  ...
  const { data: vendors, isLoading: vendorsLoading } = useQuery({
    queryKey: ["restaurant-vendors", country],
    queryFn: async () => {
        ...
        <div className="flex items-center justify-between gap-4 py-1">
          <div className="flex items-center gap-3 shrink-0">
            <div className="min-w-0">
              <div className="text-xl font-bold truncate text-zinc-900">Restaurants</div>
            </div>
          </div>
    ...
    >
      <div className="pt-3 w-full max-w-2xl lg:max-w-6xl mx-auto">
        <div className="space-y-8">
          <h1 className="hidden lg:block font-display text-2xl font-bold tracking-tight">Restaurants</h1>
          ...
          <section>
            ...
            {vendorsLoading ? (
              ...
              <EmptyState title="No restaurants yet" hint={`Check back soon as we expand in ${country === "NG" ? "Nigeria" : "the UK"}.`} />
            ) : (
          ...
          {otherStores.length > 0 && (
            <section className="mt-8">
              <SectionHeader title={q ? "Search results" : "More Restaurants"} />
